...
import org.col.matching.NameMatcher;
...
import org.col.tasks.old.model.NameUsageNode;
import org.col.tasks.old.model.RankedName;
import org.col.tasks.old.neo.*;
import org.col.tasks.importer.neo.traverse.Traversals;
import org.col.tasks.importer.neo.traverse.TreeWalker;
...
public class Normalizer extends ImportDb implements Runnable {
  ...
  private Normalizer(int datasetKey, UsageDao dao, File dwca, int batchSize, Map<String, UUID> constituents, NameMatcher matcher) {
   ...
   * to load the archive.
   */
  public static Normalizer create(WsConfig cfg, int datasetKey, Map<String, UUID> constituents, NameMatcher lookup) {
    return new Normalizer(datasetKey,
            UsageDao.persistentDao(cfg.neo, datasetKey, true),
            cfg.archiveDir(datasetKey),
            cfg.neo.batchSize,
            constituents, lookup);
  }

  public static Normalizer create(int datasetKey, UsageDao dao, File dwca, NameMatcher lookup, int batchSize) {
    return new Normalizer(datasetKey, dao, dwca, batchSize, Maps.newHashMap(), lookup);
  }
  ...
  private void buildMetricsAndMatchBackbone() throws NormalizationFailedException {
    ...
    matchHandler = new NubMatchHandler(matcher, dao);
    ...
    if (unsplitIds != null) {
      if (unsplitIds.equals(nn.usage.getTaxonID())) {
        ...
        if (meta.getMultiValueDelimiters().containsKey(DwcTerm.acceptedNameUsageID)) {
          ...
          // matcher by taxon id to see if this is an existing identifier or if we should try to split it
